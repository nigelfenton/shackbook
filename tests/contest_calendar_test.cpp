...
int main(int argc, char** argv)
{
    ...
    ContestCalendar cal;
    const int n = cal.load(QStringLiteral(SHACKLOG_CALENDAR_DAT));
    ...
    {
        const QString tmp = QDir::tempPath() + QStringLiteral("/shacklog_cal_ny_test.dat");
        QFile f(tmp);
    ...
    {
        const QString tmp = QDir::tempPath() + QStringLiteral("/shacklog_cal_bad_test.dat");
        QFile f(tmp);
