...
Where ShackBook is headed. Nothing here is a promise or a schedule — it's the
...
- **Native CAT, without Hamlib** — v0.6.0 reaches non-TCI radios through Hamlib's
  ...
  - ⚠ Keep the `rigctld` path as the escape hatch. It covers the long tail of
    older and stranger radios, and — the real argument — it lets ShackBook and
    WSJT-X share one radio instead of fighting over one COM port.
- **Session Map** ([#3](https://github.com/nigelfenton/shackbook/issues/3)) —
...
- **Cabrillo validation** — sanity-check a log against the selected contest's
  rules before export.
- **QSO party mode** ([#4](https://github.com/nigelfenton/shackbook/issues/4)) —
  ...
  ([#5](https://github.com/nigelfenton/shackbook/issues/5)), without which the
...
- **Live remote-log mode** — view and log against a running `shackbook-server`
...
[GitHub](https://github.com/nigelfenton/shackbook/issues).
