@echo off
REM Build + run the ShackLog unit tests (opt-in target set).
...
if not exist build-tests\CMakeCache.txt (
    ...
    cmake -B build-tests -G Ninja -DCMAKE_BUILD_TYPE=Release -DSHACKLOG_TESTS=ON -DCMAKE_PREFIX_PATH="C:/Qt/6.10.3/msvc2022_64"
